...
timeframe.  New features or functionality added post-Beta 1 have not been incorporated into the Nada.DA provider
...
not implemented and/or are stubbed out in the Nada.DA provider source code:
...
The providers all carry out read and write operations against the Nada.DA database using the security 
...
enforced by the Nada.DA providers.
...
The Nada.DA provider does not support creating a new user with an explicit UserId (i.e. providerUserKey).

The Nada.DA provider does not support retrieving a user based on UserId (i.e. providerUserKey).
