...
//        }
        ...
        public List<string> GetSchemaChangeScripts(string scriptsDirectory)
        {
            ...
            using (connection)
            {
                ...
                try
                {
                    OleDbCommand command = new OleDbCommand("Select top 1 ScriptName from SchemaChangeLog Order By MajorReleaseNumber DESC, MinorReleaseNumber DESC, PointReleaseNumber DESC", connection);
                    using (OleDbDataReader reader = command.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
