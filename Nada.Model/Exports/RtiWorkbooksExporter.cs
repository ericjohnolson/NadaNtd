    ...
    public class RtiWorkbooksExporter : ExporterBase, IExporter
    {
        ...
        private void AddLf(excel.Worksheet xlsWorksheet, excel.Range rng, DateTime start, DateTime end, List<AdminLevel> demography, Dictionary<int, DataRow> aggIntvs)
        {
            ...
            foreach (var unit in demography)
            {
                ...
                {
                    string endemicity = ParseLfDdEnd(lfDd[unit.Id][transLookup.GetValue("DDLFDiseaseDistributionPcInterventions") + " - " + lf.Disease.DisplayName].ToString());
                    AddValueToRange(xlsWorksheet, rng, "F" + rowCount, endemicity);

                    AddValueToRange(xlsWorksheet, rng, "J" + rowCount,
                        ParseTasObjective(lfDd[unit.Id][transLookup.GetValue("DDLFObjectiveOfPlannedTas") + " - " + lf.Disease.DisplayName].ToString()));
                    AddValueToRange(xlsWorksheet, rng, "K" + rowCount,
                        lfDd[unit.Id][transLookup.GetValue("DDLFMonthOfPlannedTas") + " - " + lf.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "L" + rowCount,
                        lfDd[unit.Id][transLookup.GetValue("DDLFYearOfPlannedTas") + " - " + lf.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "M" + rowCount,
                        lfDd[unit.Id][transLookup.GetValue("DDLFPopulationAtRisk") + " - " + lf.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "N" + rowCount,
                        lfDd[unit.Id][transLookup.GetValue("DDLFPopulationRequiringPc") + " - " + lf.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "O" + rowCount,
                        lfDd[unit.Id][transLookup.GetValue("DDLFPopulationLivingInTheDistrictsThatAc") + " - " + lf.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "P" + rowCount,
                        lfDd[unit.Id][transLookup.GetValue("DDLFYearDeterminedThatAchievedCriteriaFo") + " - " + lf.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "T" + rowCount,
                        lfDd[unit.Id][transLookup.GetValue("DDLFYearPcStarted") + " - " + lf.Disease.DisplayName]);
                ...
                if (aggIntvs.ContainsKey(unit.Id))
                {
                    ...
                    if (aggIntvs[unit.Id].Table.Columns.Contains(transLookup.GetValue("LFMMDPNumLymphoedemaPatients") + " - " + transLookup.GetValue("LfMorbidityManagment")))
                        AddValueToRange(xlsWorksheet, rng, "Q" + rowCount,
                            aggIntvs[unit.Id][transLookup.GetValue("LFMMDPNumLymphoedemaPatients") + " - " + transLookup.GetValue("LfMorbidityManagment")]);
                    if (aggIntvs[unit.Id].Table.Columns.Contains(transLookup.GetValue("LFMMDPNumHydroceleCases") + " - " + transLookup.GetValue("LfMorbidityManagment")))
                        AddValueToRange(xlsWorksheet, rng, "R" + rowCount,
                            aggIntvs[unit.Id][transLookup.GetValue("LFMMDPNumHydroceleCases") + " - " + transLookup.GetValue("LfMorbidityManagment")]);
        ...
        private void AddOncho(excel.Worksheet xlsWorksheet, excel.Range rng, DateTime start, DateTime end, List<AdminLevel> demography, Dictionary<int, DataRow> aggIntvs)
        {
            ...
            foreach (var unit in demography)
            {
                ...
                {
                    string endemicity = ParseOnchoDdEndo(onchoDd[unit.Id][transLookup.GetValue("DDOnchoDiseaseDistributionPcInterventio") + " - " + oncho.Disease.DisplayName].ToString());
                    AddValueToRange(xlsWorksheet, rng, "G" + rowCount, endemicity);

                    AddValueToRange(xlsWorksheet, rng, "K" + rowCount,
                        onchoDd[unit.Id][transLookup.GetValue("DDOnchoPopulationAtRisk") + " - " + oncho.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "L" + rowCount,
                        onchoDd[unit.Id][transLookup.GetValue("DDOnchoPopulationRequiringPc") + " - " + oncho.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "M" + rowCount,
                        onchoDd[unit.Id][transLookup.GetValue("DDOnchoPopulationLivingInTheDistrictsTha") + " - " + oncho.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "N" + rowCount,
                        onchoDd[unit.Id][transLookup.GetValue("DDOnchoYearDeterminedThatAchievedCriteri") + " - " + oncho.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "O" + rowCount,
                        onchoDd[unit.Id][transLookup.GetValue("DDOnchoNumPcRoundsYearCurrentlyImplemen") + " - " + oncho.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "Q" + rowCount,
                        onchoDd[unit.Id][transLookup.GetValue("DDOnchoYearPcStarted") + " - " + oncho.Disease.DisplayName]);
                }
        ...
        private void AddSchisto(excel.Worksheet xlsWorksheet, excel.Range rng, DateTime start, DateTime end, List<AdminLevel> demography, Dictionary<int, DataRow> aggIntvs)
        {
            ...
            foreach (var unit in demography)
            {
                ...
                {
                    string endemicity = ParseSchEnd(schDd[unit.Id][transLookup.GetValue("DDSchistoDiseaseDistributionPcIntervent") + " - " + sch.Disease.DisplayName].ToString());
                    AddValueToRange(xlsWorksheet, rng, "H" + rowCount, endemicity);
                    AddValueToRange(xlsWorksheet, rng, "P" + rowCount,
                        schDd[unit.Id][transLookup.GetValue("DDSchistoPopulationAtRisk") + " - " + sch.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "Q" + rowCount,
                        schDd[unit.Id][transLookup.GetValue("DDSchistoPopulationRequiringPc") + " - " + sch.Disease.DisplayName]);

                    AddValueToRange(xlsWorksheet, rng, "R" + rowCount,
                       schDd[unit.Id][transLookup.GetValue("DDSchistoSacAtRisk") + " - " + sch.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "S" + rowCount,
                       schDd[unit.Id][transLookup.GetValue("DDSchistoHighRiskAdultsAtRisk") + " - " + sch.Disease.DisplayName]);

                    AddValueToRange(xlsWorksheet, rng, "T" + rowCount,
                        schDd[unit.Id][transLookup.GetValue("DDSchistoPopulationLivingInTheDistrictsT") + " - " + sch.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "U" + rowCount,
                        schDd[unit.Id][transLookup.GetValue("DDSchistoYearDeterminedThatAchievedCrite") + " - " + sch.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "V" + rowCount,
                        ParseSchistoFrequency(schDd[unit.Id][transLookup.GetValue("DDSchistoNumPcRoundsYearRecommendedByWh") + " - " + sch.Disease.DisplayName].ToString()));
                    AddValueToRange(xlsWorksheet, rng, "W" + rowCount,
                        ParseSchistoFrequency(schDd[unit.Id][transLookup.GetValue("DDSchistoNumPcRoundsYearCurrentlyImplem") + " - " + sch.Disease.DisplayName].ToString()));
                    AddValueToRange(xlsWorksheet, rng, "Y" + rowCount,
                        schDd[unit.Id][transLookup.GetValue("DDSchistoYearPcStarted") + " - " + sch.Disease.DisplayName]);
                }
                // MDA count
                int mdas = 0, consecutive = 0;
        ...
        private void AddSth(excel.Worksheet xlsWorksheet, excel.Range rng, DateTime start, DateTime end, List<AdminLevel> demography, Dictionary<int, DataRow> aggIntvs)
        {
            ...
            foreach (var unit in demography)
            {
                ...
                {
                    string endemicity = ParseSthEnd(sthDd[unit.Id][transLookup.GetValue("DDSTHDiseaseDistributionPcInterventions") + " - " + sth.Disease.DisplayName].ToString());
                    AddValueToRange(xlsWorksheet, rng, "H" + rowCount, endemicity);
                    AddValueToRange(xlsWorksheet, rng, "P" + rowCount,
                        sthDd[unit.Id][transLookup.GetValue("DDSTHPopulationAtRisk") + " - " + sth.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "Q" + rowCount,
                        sthDd[unit.Id][transLookup.GetValue("DDSTHPopulationRequiringPc") + " - " + sth.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "R" + rowCount,
                       sthDd[unit.Id][transLookup.GetValue("DDSTHSacAtRisk") + " - " + sth.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "S" + rowCount,
                       sthDd[unit.Id][transLookup.GetValue("DDSTHHighRiskAdultsAtRisk") + " - " + sth.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "T" + rowCount,
                       sthDd[unit.Id][transLookup.GetValue("DDSTHPopulationLivingInTheDistrictsThatA") + " - " + sth.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "U" + rowCount,
                        sthDd[unit.Id][transLookup.GetValue("DDSTHYearDeterminedThatAchievedCriteriaF") + " - " + sth.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "V" + rowCount,
                        ParseSthFrequency(sthDd[unit.Id][transLookup.GetValue("DDSTHNumPcRoundsYearRecommendedByWhoGui") + " - " + sth.Disease.DisplayName].ToString()));
                    AddValueToRange(xlsWorksheet, rng, "W" + rowCount,
                        ParseSthFrequency(sthDd[unit.Id][transLookup.GetValue("DDSTHNumPcRoundsYearCurrentlyImplemente") + " - " + sth.Disease.DisplayName].ToString()));
                    AddValueToRange(xlsWorksheet, rng, "Y" + rowCount,
                        sthDd[unit.Id][transLookup.GetValue("DDSTHYearPcStarted") + " - " + sth.Disease.DisplayName]);
                }
                // MDA count
                int mdas = 0, consecutive = 0;
        ...
        private void AddTrachoma(excel.Worksheet xlsWorksheet, excel.Range rng, DateTime start, DateTime end, List<AdminLevel> demography, Dictionary<int, DataRow> aggIntvs)
        {
            ...
            foreach (var unit in demography)
            {
                ...
                {
                    string endemicity = ParseTrachomaEnd(traDd[unit.Id][transLookup.GetValue("DDTraDiseaseDistributionPcInterventions") + " - " + tra.Disease.DisplayName].ToString());
                    AddValueToRange(xlsWorksheet, rng, "F" + rowCount, endemicity);
                    AddValueToRange(xlsWorksheet, rng, "J" + rowCount,
                        traDd[unit.Id][transLookup.GetValue("DDTraTrichiasisBacklog") + " - " + tra.Disease.DisplayName]);

                    DateTime datePlanned;
                    if (DateTime.TryParseExact(traDd[unit.Id][transLookup.GetValue("DDTraYearOfPlannedTrachomaImpactSurvey") + " - " + tra.Disease.DisplayName].ToString(),
                    ...
                    AddValueToRange(xlsWorksheet, rng, "Q" + rowCount,
                    traDd[unit.Id][transLookup.GetValue("DDTraPopulationAtRisk") + " - " + tra.Disease.DisplayName]);

                    string achievedCriteria = traDd[unit.Id][transLookup.GetValue("DDTraAchievedCriteria") + " - " + tra.Disease.DisplayName].ToString();
                    if (achievedCriteria == transLookup.GetValue("Yes"))
                        AddValueToRange(xlsWorksheet, rng, "T" + rowCount, achievedCriteria);
                    AddValueToRange(xlsWorksheet, rng, "U" + rowCount,
                       traDd[unit.Id][transLookup.GetValue("DDTraYearDeterminedThatAchievedCriteriaF") + " - " + tra.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "V" + rowCount,
                       traDd[unit.Id][transLookup.GetValue("DDTraPopulationLivingInAreasDistrict") + " - " + tra.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "W" + rowCount,
                       traDd[unit.Id][transLookup.GetValue("DDTraYearAchievedUiga") + " - " + tra.Disease.DisplayName]);
                    AddValueToRange(xlsWorksheet, rng, "X" + rowCount,
                       traDd[unit.Id][transLookup.GetValue("DDTraPopulationLivingInAreasSubDistrict") + " - " + tra.Disease.DisplayName]);
                }
                // MDA count
                int mdas = 0, consecutive = 0;
                ...
                if (aggIntvs.ContainsKey(unit.Id))
                {
                    ...
                    if (subDistrictEligible.ContainsKey(unit.Id))
                    {
                        ...
                        if (!string.IsNullOrEmpty(belowDistrictEligible))
                            AddValueToRange(xlsWorksheet, rng, "S" + rowCount, transLookup.GetValue("RtiSubDistrict", "RtiSubDistrict"));
                    }
                    else if (!string.IsNullOrEmpty(aggEligible))
                        AddValueToRange(xlsWorksheet, rng, "S" + rowCount, transLookup.GetValue("RtiDistrict", "RtiDistrict"));
        ...
        private void AddTraLevelToRange(excel.Worksheet xlsWorksheet, excel.Range rng, int rowCount, SurveyBase mostRecentSurvey, string indName, string colName)
        {
            ...
            if (ind != null && !string.IsNullOrEmpty(ind.DynamicValue))
            {
                if (ind.DynamicValue == "TraSurDistLevel")
                    AddValueToRange(xlsWorksheet, rng, colName + rowCount, transLookup.GetValue("RtiDistrict", "RtiDistrict"));
                else if (ind.DynamicValue == "TraSurSubDistLevel")
                    AddValueToRange(xlsWorksheet, rng, colName + rowCount, transLookup.GetValue("RtiSubDistrict", "RtiSubDistrict"));
            }
        }
        ...
        {
            if (endo == transLookup.GetValue("Oncho0"))
                return transLookup.GetValue("RtiLfDd0", "RtiLfDd0");
            if (endo == transLookup.GetValue("Oncho1"))
                return transLookup.GetValue("RtiLfDd1", "RtiLfDd1");
            if (endo == transLookup.GetValue("OnchoM"))
                return transLookup.GetValue("RtiLfDdM", "RtiLfDdM");
            if (endo == transLookup.GetValue("OnchoNs"))
                return transLookup.GetValue("RtiLfDdNs", "RtiLfDdNs");
            if (endo == transLookup.GetValue("Oncho100"))
                return transLookup.GetValue("RtiLfDd100", "RtiLfDd100");
        ...
        {
            if (tas == transLookup.GetValue("LfPostMdaTas1"))
                return transLookup.GetValue("RtiPostMda1", "RtiPostMda1");
            if (tas == transLookup.GetValue("LfPostMdaTas2"))
        ...
        {
            if (freq == transLookup.GetValue("xyear2"))
                return transLookup.GetValue("RtiSchMdaD", "RtiSchMdaD");
            if (freq == transLookup.GetValue("xyear1every2"))
                return transLookup.GetValue("RtiSchMdaB", "RtiSchMdaB");
            if (freq == transLookup.GetValue("xyear1every3"))
                return transLookup.GetValue("RtiSchMdaC", "RtiSchMdaC");
            if (freq == transLookup.GetValue("xyear1"))
        ...
        private string ParseSthFrequency(string freq)
        {
            ...
            if (freq == transLookup.GetValue("xyear1every2"))
                return transLookup.GetValue("RtiSchMdaC", "RtiSchMdaC");
            if (freq == transLookup.GetValue("xyear3"))
                return transLookup.GetValue("RtiSchMdaD", "RtiSchMdaD");
            if (freq == transLookup.GetValue("xyear1"))
                return transLookup.GetValue("RtiSchMdaB", "RtiSchMdaB");
        ...
        {
            if (end == transLookup.GetValue("SthM"))
                return transLookup.GetValue("RtiSchM", "RtiSchM");
            if (end == transLookup.GetValue("SthNs"))
                return transLookup.GetValue("RtiSchNs", "RtiSchNs");
            if (end == transLookup.GetValue("Sth0"))
                return transLookup.GetValue("RtiSch0", "RtiSch0");
            if (end == transLookup.GetValue("Sth1"))
                return transLookup.GetValue("RtiSch1", "RtiSch1");
            if (end == transLookup.GetValue("Sth2"))
                return transLookup.GetValue("RtiSch2", "RtiSch2");
            if (end == transLookup.GetValue("Sth3"))
                return transLookup.GetValue("RtiSch3", "RtiSch3");
            if (end == transLookup.GetValue("Sth10"))
                return transLookup.GetValue("RtiSch10", "RtiSch10");
            if (end == transLookup.GetValue("Sth20"))
                return transLookup.GetValue("RtiSch20", "RtiSch20");
            if (end == transLookup.GetValue("Sth30"))
                return transLookup.GetValue("RtiSch30", "RtiSch30");
            if (end == transLookup.GetValue("Sth40"))
                return transLookup.GetValue("RtiSch40", "RtiSch40");
            if (end == transLookup.GetValue("Sth100"))
                return transLookup.GetValue("RtiSch100", "RtiSch100");
